...
class Converter
{
    ...
    public function convert(array $data): array
    {
        $meta = $data['__meta'];
        // Default output
        $output = ['id' => $meta['id'], 'method' => $meta['method'], 'uri' => $meta['uri'], 'time' => $meta['utime'], 'headers' => [], 'cookies' => [], 'emailsData' => [], 'getData' => [], 'log' => [], 'postData' => [], 'sessionData' => [], 'timelineData' => [], 'viewsData' => [], 'controller' => null, 'responseTime' => null, 'responseStatus' => null, 'responseDuration' => 0];
        ...
        if (isset($data['time']['measures'])) {
            ...
            foreach ($time['measures'] as $measure) {
                $output['timelineData'][] = ['data' => [], 'description' => $measure['label'], 'duration' => $measure['duration'] * 1000, 'end' => $measure['end'], 'start' => $measure['start'], 'relative_start' => $measure['start'] - $time['start']];
            }
        }
        if (isset($data['route'])) {
            $route = $data['route'];
            ...
            $output['routes'][] = ['action' => $output['controller'], 'after' => $route['after'] ?? null, 'before' => $route['before'] ?? null, 'method' => $method, 'name' => $route['as'] ?? null, 'uri' => $uri];
        }
        if (isset($data['messages']['messages'])) {
            foreach ($data['messages']['messages'] as $message) {
                $output['log'][] = ['message' => $message['message'], 'time' => $message['time'], 'level' => $message['label']];
            }
        }
        if (isset($data['queries']['statements'])) {
            ...
            foreach ($queries['statements'] as $statement) {
                ...
                $output['databaseQueries'][] = ['query' => $statement['sql'], 'bindings' => $statement['params'], 'duration' => $statement['duration'] * 1000, 'time' => $statement['start'] ?? null, 'connection' => $statement['connection']];
            }
        ...
        if (isset($data['models']['data'])) {
            ...
            foreach ($data['models']['data'] as $model => $value) {
                foreach ($value as $event => $count) {
                    $event_key = 'models' . ucfirst((string) $event);
                    if (isset($output[$event_key])) {
                        $output[$event_key][$model] = $count;
                    }
                }
            }
        }
        if (isset($data['views']['templates'])) {
            foreach ($data['views']['templates'] as $view) {
                $output['viewsData'][] = ['description' => 'Rendering a view', 'duration' => 0, 'end' => 0, 'start' => $view['start'] ?? 0, 'data' => ['name' => $view['name'], 'data' => $view['params']]];
            }
        }
        if (isset($data['event']['measures'])) {
            foreach ($data['event']['measures'] as $event) {
                ...
                $output['events'][] = ['event' => ['event' => $event['label']], 'data' => $event['data'], 'time' => $event['start'], 'duration' => $event['duration'] * 1000, 'listeners' => $event['listeners']];
            }
        }
        if (isset($data['symfonymailer_mails']['mails'])) {
            foreach ($data['symfonymailer_mails']['mails'] as $mail) {
                $output['emailsData'][] = ['data' => ['to' => implode(', ', $mail['to']), 'subject' => $mail['subject'], 'headers' => isset($mail['headers']) ? explode("\n", $mail['headers']) : null]];
            }
        }
        return $output;
    }
